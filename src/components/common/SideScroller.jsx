...
const SideScroller = () => {
  ...
  return (
    ...
    // Fixed at vertical center (top-1/2) on the right edge (right-0)
    <div 
      className="fixed right-0 top-1/2 -translate-y-1/2 z-50 hidden md:flex items-center"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      {/* 1. THE EXPANDABLE MENU (Left side of container) */}
      <AnimatePresence>
        {isHovered && (
          <motion.div
            ...
            className="mr-2" // Spacing between menu and handle
          >
      ...
      {/* 2. THE TRIGGER HANDLE (Right side of container) */}
      <motion.div
        animate={{ 
            x: isHovered ? 10 : 0, // Slides out slightly when hovered
      ...
      {/* Invisible buffer to ensure smooth mouse transition */}
